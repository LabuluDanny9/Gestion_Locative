-- Phase 8: secure, idempotent rent invoice generation and status refresh.
...
begin
  ...
  -- Time changes even when no invoice row is edited, so refresh derived states first.
  ...
  loop
    ...
    loop
      v_period_end := (v_period_start + make_interval(months => v_interval_months) - 1)::date;
...
create or replace function public.create_lease_and_invoices(
  p_organization_id uuid,
  p_tenant_id uuid,
  p_unit_id uuid,
  p_start_date date,
...
begin
  v_lease_id := public.create_lease_with_tenant(
    p_organization_id, p_tenant_id, p_unit_id, p_start_date, p_end_date,
...
revoke all on function public.create_lease_and_invoices(uuid, uuid, uuid, date, date, numeric, public.currency_code, numeric, numeric, public.billing_frequency, smallint, text) from public, anon;
grant execute on function public.create_lease_and_invoices(uuid, uuid, uuid, date, date, numeric, public.currency_code, numeric, numeric, public.billing_frequency, smallint, text) to authenticated;
