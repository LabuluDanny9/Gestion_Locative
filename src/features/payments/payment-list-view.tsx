...
export type PaymentListParams = { q?: string; status?: string; mode?: string; currency?: "USD" | "CDF"; creation?: string; erreur?: string };
...
export function PaymentListView({ basePath, dashboardHref, params, receiptBasePath, payments = [] }: { basePath: string; dashboardHref: string; params: PaymentListParams; receiptBasePath: string; payments?: Payment[] }) {
  ...
  const currency = params.currency ?? payments[0]?.currency ?? "USD";
  const currencyPayments = payments.filter((payment) => payment.currency === currency);
  const filtered = currencyPayments.filter((payment) => (!query || `${payment.reference} ${payment.tenantName} ${payment.unitLabel} ${payment.receiptNumber}`.toLocaleLowerCase("fr").includes(query)) && (!params.status || params.status === "all" || payment.status === params.status) && (!params.mode || params.mode === "all" || payment.mode === params.mode));
  ...
  const completed = currencyPayments.filter((payment) => payment.status !== "cancelled");
  const todayPayments = completed.filter((payment) => payment.date === todayLabel);
  const monthPayments = completed.filter((payment) => monthFormatter.format(new Date(payment.paidAtIso)) === monthLabel);
  ...
  return <div className="space-y-6"><div><Breadcrumbs items={[{ label: "Dashboard", href: dashboardHref }, { label: "Paiements" }]} /><PageHeader actions={<Button asChild><Link href={`${basePath}/nouveau`}><CirclePlus />Nouveau paiement</Link></Button>} description="Enregistrez les encaissements, contrôlez leur affectation et accédez immédiatement aux reçus." eyebrow="Finances" title="Paiements" /></div><section aria-label="Synthèse des paiements" className="grid grid-cols-2 gap-3 xl:grid-cols-4">{[
    { label: "Encaissé aujourd’hui", value: `${new Intl.NumberFormat("fr-CD").format(todayPayments.reduce((sum, payment) => sum + payment.amount, 0))} ${currency}`, icon: CircleDollarSign },
    { label: "Encaissé ce mois", value: `${new Intl.NumberFormat("fr-CD").format(monthPayments.reduce((sum, payment) => sum + payment.amount, 0))} ${currency}`, icon: WalletCards },
    { label: "Paiements du jour", value: todayPayments.length, icon: ListChecks },
    { label: "Paiements partiels", value: currencyPayments.filter((payment) => payment.status === "partial").length, icon: CircleDollarSign },
  ].map(({ label, value, icon: Icon }) => <Card key={label}><CardContent className="flex items-center gap-3 p-4"><span className="grid size-9 place-items-center rounded-xl bg-brand-blue/10 text-brand-blue"><Icon className="size-4.5" /></span><div className="min-w-0"><p className="truncate text-xs text-muted-foreground">{label}</p><p className="mt-0.5 truncate font-heading text-lg font-semibold tabular-nums">{value}</p></div></CardContent></Card>)}</section><FilterBar><form action={basePath} className="grid flex-1 gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(16rem,1fr)_8rem_12rem_13rem_auto]" method="GET"><label className="relative"><span className="sr-only">Rechercher un paiement</span><Search className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" defaultValue={params.q} name="q" placeholder="Référence, locataire, reçu..." /></label><select aria-label="Filtrer par devise" className="h-10 rounded-lg border bg-background px-3 text-sm" defaultValue={currency} name="currency"><option value="USD">USD</option><option value="CDF">CDF</option></select><select aria-label="Filtrer par statut" className="h-10 rounded-lg border bg-background px-3 text-sm" defaultValue={params.status ?? "all"} name="status"><option value="all">Tous les statuts</option><option value="paid">Payés</option><option value="partial">Partiels</option><option value="cancelled">Annulés</option></select><select aria-label="Filtrer par mode" className="h-10 rounded-lg border bg-background px-3 text-sm" defaultValue={params.mode ?? "all"} name="mode"><option value="all">Tous les modes</option><option value="cash">Espèces</option><option value="mobile">Mobile Money</option><option value="bank">Banque</option><option value="card">Carte</option></select><Button type="submit" variant="secondary"><ListChecks />Appliquer</Button></form></FilterBar>{filtered.length === 0 ? <EmptyState action={<Button asChild variant="outline"><Link href={basePath}>Effacer les filtres</Link></Button>} description="Modifiez les filtres ou enregistrez un nouvel encaissement." icon={WalletCards} title="Aucun paiement trouvé" /> : <><div className="grid gap-4 lg:hidden">{filtered.map((payment) => <PaymentCard href={`${basePath}/${payment.id}`} key={payment.id} payment={payment} />)}</div><div className="hidden overflow-x-auto lg:block"><DataTable className="min-w-[82rem]" columns={columns} getRowKey={(payment) => payment.id} rows={filtered} /></div></>}</div>;
}
