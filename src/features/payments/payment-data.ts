...
export type Payment = { id: string; reference: string; receiptId: string; receiptNumber: string; tenantId: string; tenantName: string; unitId: string; unitLabel: string; propertyName: string; period: string; amount: number; currency: "USD" | "CDF"; mode: PaymentMode; date: string; paidAtIso: string; time: string; status: PaymentStatus; agent: string; balanceBefore: number; balanceAfter: number; allocations: PaymentAllocation[]; note?: string; reversalReason?: string; reversedAt?: string; reversedBy?: string };
