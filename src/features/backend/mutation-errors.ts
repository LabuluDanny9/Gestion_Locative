...
export function mutationMessage(cause: unknown) {
  ...
  if (message.includes("Photo invalide") || message.includes("Document invalide") || message.includes("échéances contractuelles")) return message;
  if (/TextBee|Infobip|WhatsApp|textbee|meta_whatsapp_cloud|Numéro de téléphone invalide/i.test(message)) return message.slice(0, 500);
