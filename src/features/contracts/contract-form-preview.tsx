...
export function ContractFormPreview({ basePath, dashboardHref }: { basePath: string; dashboardHref: string }) {
  ...
  return <div className="space-y-6"><div><Breadcrumbs items={[{ label: "Dashboard", href: dashboardHref }, { label: "Contrats", href: basePath }, { label: "Nouveau contrat" }]} /><PageHeader description="Préparez le bail étape par étape, des parties jusqu’aux documents et à la vérification." eyebrow="Assistant contractuel" title="Créer un contrat" /></div><Badge variant="secondary">Prototype interactif · aucune donnée enregistrée</Badge>
    ...
    <form className="mx-auto max-w-5xl" onSubmit={handleSubmit} ref={formRef}>
      <WizardSection active={step === 0} description="Sélectionnez le titulaire du futur bail." icon={UserRound} step={0} title="Locataire"><div className="grid gap-3 sm:grid-cols-2">{tenants.map((tenant) => <label className="flex cursor-pointer items-center gap-3 rounded-xl border p-4 has-checked:border-brand-blue has-checked:bg-brand-blue/5" key={tenant.id}><input className="size-4 accent-brand-blue" name="tenant" required type="radio" value={tenant.id} /><TenantAvatar name={tenant.name} /><div><span className="font-medium">{tenant.name}</span><span className="mt-0.5 block text-xs text-muted-foreground">{tenant.phone}</span></div></label>)}</div></WizardSection>
      <WizardSection active={step === 1} description="Choisissez l’unité concernée par le contrat." icon={House} step={1} title="Logement"><div className="grid gap-3 sm:grid-cols-2">{units.map((unit) => <label className="flex cursor-pointer items-center gap-3 rounded-xl border p-4 has-checked:border-brand-blue has-checked:bg-brand-blue/5" key={unit.id}><input className="size-4 accent-brand-blue" name="unit" required type="radio" value={unit.id} /><House className="size-5 text-brand-blue" /><div><span className="font-medium">{unit.type} {unit.code}</span><span className="mt-0.5 block text-xs text-muted-foreground">{unit.propertyName} · {unit.rent} {unit.currency}</span></div></label>)}</div></WizardSection>
      ...
      <div className="sticky bottom-4 mt-5 flex flex-col-reverse justify-between gap-2 rounded-xl border bg-background/95 p-3 shadow-lg backdrop-blur sm:flex-row"><div>{step === 0 ? <Button asChild variant="outline"><Link href={basePath}>Annuler</Link></Button> : <Button onClick={() => setStep((current) => current - 1)} type="button" variant="outline"><ChevronLeft />Précédent</Button>}</div>{step < steps.length - 1 ? <Button onClick={nextStep} type="button">Continuer <ChevronRight /></Button> : <Button type="submit"><CheckCircle2 />Valider l’aperçu</Button>}</div>
    </form></div>;
}
