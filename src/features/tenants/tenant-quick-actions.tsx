...
export function TenantQuickActions({ contractHref, paymentHref, tenantId, tenantName, deleteAction, messageAction }: { contractHref: string; paymentHref: string; tenantId: string; tenantName: string; deleteAction?: (form: FormData) => void | Promise<void>; messageAction?: (form: FormData) => void | Promise<void> }) {
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {messageAction ? <Dialog><DialogTrigger asChild><Button variant="outline"><MessageCircle />Envoyer message</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Écrire à {tenantName}</DialogTitle><DialogDescription>Le message sera envoyé par Infobip et la tentative sera conservée dans le journal des notifications.</DialogDescription></DialogHeader><form action={messageAction} className="space-y-4"><input name="tenantId" type="hidden" value={tenantId} /><div className="space-y-2"><Label htmlFor="message-channel">Canal</Label><select className="h-10 w-full rounded-lg border bg-background px-3 text-sm" defaultValue="sms" id="message-channel" name="channel"><option value="sms">SMS — Infobip</option><option value="whatsapp">WhatsApp — Infobip</option></select></div><div className="space-y-2"><Label htmlFor="tenant-message">Message</Label><Textarea id="tenant-message" maxLength={500} minLength={2} name="body" placeholder="Bonjour, votre message…" required rows={6} /><p className="text-xs text-muted-foreground">WhatsApp nécessite un expéditeur Infobip et un modèle approuvé.</p></div><DialogFooter><MessageSubmit /></DialogFooter></form></DialogContent></Dialog> : null}
