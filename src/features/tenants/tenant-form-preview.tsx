  ...
  return (
    <div className="space-y-6">
      ...
      <form action={action} className="mx-auto max-w-5xl space-y-5" onSubmit={action ? undefined : handleSubmit}>
        ...
        <Card><CardHeader><CardTitle>Documents</CardTitle><CardDescription>{action ? "Les documents seront stockés dans un espace privé." : "Les fichiers seront reliés à Supabase Storage lors de la phase backend."}</CardDescription></CardHeader><CardContent><label className="flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed bg-muted/25 p-6 text-center transition-colors hover:border-brand-blue/40 hover:bg-brand-blue/5"><CloudUpload aria-hidden="true" className="size-8 text-brand-blue" /><span className="mt-3 font-medium">Glissez vos fichiers ici ou cliquez pour parcourir</span><span className="mt-1 text-xs text-muted-foreground">Pièce d’identité, photo ou justificatif</span><Input className="sr-only" multiple name="documents" type="file" /></label></CardContent></Card>
        <div className="flex flex-col-reverse justify-end gap-2 sm:flex-row"><Button asChild variant="outline"><Link href={basePath}>Annuler</Link></Button><Button type="submit"><Save />{action ? "Créer le locataire" : "Valider l’aperçu"}</Button></div>
      </form>
    </div>
  );
}
