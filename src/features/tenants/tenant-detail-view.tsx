...
export function TenantDetailView({ tenant, basePath, dashboardHref, unitBasePath, contractBasePath, paymentBasePath, receiptBasePath, deleteAction, messageAction }: { tenant: Tenant; basePath: string; dashboardHref: string; unitBasePath: string; contractBasePath: string; paymentBasePath: string; receiptBasePath: string; deleteAction?: (form: FormData) => void | Promise<void>; messageAction?: (form: FormData) => void | Promise<void> }) {
  const kpis = [
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-5 rounded-2xl border bg-card p-5 sm:p-6 xl:flex-row xl:items-center xl:justify-between">
        ...
        <TenantQuickActions contractHref={`${contractBasePath}/${tenant.contractId}`} deleteAction={deleteAction} messageAction={messageAction} paymentHref={`${paymentBasePath}/nouveau?tenant=${tenant.id}`} tenantId={tenant.id} tenantName={tenant.name} />
      </header>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent className="mt-5" value="documents"><Card><CardHeader><CardTitle>Documents du dossier</CardTitle><CardDescription>Pièces privées enregistrées dans Supabase Storage.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{tenant.documents.length ? tenant.documents.map((document) => <DocumentPreview icon={IdCard} key={document.id} label={document.name} url={document.url} />) : <p className="text-sm text-muted-foreground">Aucun document téléversé.</p>}</CardContent></Card></TabsContent>
...
function DocumentPreview({ icon: Icon, label, url }: { icon: typeof IdCard; label: string; url?: string }) {
  return <div className="flex items-center gap-3 rounded-xl border p-4"><span className="grid size-10 place-items-center rounded-xl bg-brand-blue/10 text-brand-blue"><Icon aria-hidden="true" className="size-5" /></span><div className="min-w-0"><p className="truncate font-medium">{label}</p>{url ? <a className="mt-0.5 inline-block text-xs text-brand-blue hover:underline" href={url}>Télécharger</a> : <p className="mt-0.5 text-xs text-muted-foreground">Fichier indisponible</p>}</div></div>;
}
