...
export function TenantDetailView({ tenant, basePath, dashboardHref, unitBasePath, contractBasePath, paymentBasePath, receiptBasePath, deleteAction }: { tenant: Tenant; basePath: string; dashboardHref: string; unitBasePath: string; contractBasePath: string; paymentBasePath: string; receiptBasePath: string; deleteAction?: (form: FormData) => void | Promise<void> }) {
  const kpis = [
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-5 rounded-2xl border bg-card p-5 sm:p-6 xl:flex-row xl:items-center xl:justify-between">
        ...
        <TenantQuickActions contractHref={`${contractBasePath}/${tenant.contractId}`} deleteAction={deleteAction} paymentHref={`${paymentBasePath}/nouveau?tenant=${tenant.id}`} tenantId={tenant.id} tenantName={tenant.name} />
      </header>
