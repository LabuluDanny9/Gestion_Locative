...
export function UnitPhotoUploader() {
  ...
  function addPhotos(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const previews = validFiles.map((file) => {
      ...
      return { id: `${file.name}-${file.lastModified}-${crypto.randomUUID()}`, name: file.name, url };
    });
  ...
  return (
    <div className="space-y-4">
      <label className="flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed bg-muted/25 p-6 text-center transition-colors hover:border-brand-blue/40 hover:bg-brand-blue/5">
        ...
        <span className="mt-1 max-w-xl text-xs leading-5 text-muted-foreground">Salon, chambres, cuisine, salles de bain, extérieur et dépendances. JPG, PNG ou WebP · 8 Mo maximum · jusqu’à 12 photos.</span>
