  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <form action={action} className="space-y-5" onSubmit={action ? undefined : handleSubmit}>
        ...
        <div className="sticky bottom-4 flex flex-col-reverse justify-end gap-2 rounded-xl border bg-background/95 p-3 shadow-lg backdrop-blur sm:flex-row"><Button asChild variant="outline"><Link href={basePath}>Annuler</Link></Button><Button type="submit"><CheckCircle2 />{action ? "Créer le logement" : "Valider l’aperçu"}</Button></div>
      </form>
    </div>
  );
}
