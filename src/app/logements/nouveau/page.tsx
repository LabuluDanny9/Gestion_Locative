...
import { createUnitAction, finalizeUnitPhotosAction, rollbackUnitAction } from "@/features/backend/actions";
...
export default async function NewUnitPage({ searchParams }: { searchParams: Promise<{ property?: string; erreur?: string }> }) {
  ...
  return <ProtectedAppShell><MutationFeedback error={params.erreur} /><UnitFormPreview action={createUnitAction} basePath="/logements" dashboardHref="/espace" defaultProperty={params.property} finalizePhotos={finalizeUnitPhotosAction} propertyOptions={propertyOptions ?? []} rollbackUnit={rollbackUnitAction} /></ProtectedAppShell>;
}
