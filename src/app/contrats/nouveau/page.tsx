...
import { createLeaseAction, finalizeLeaseDocumentsAction, prepareLeaseDocumentUploadsAction, rollbackLeaseAction } from "@/features/backend/actions";
...
export default async function NewContractPage({ searchParams }: { searchParams: Promise<{ erreur?: string }> }) {
  ...
  return <ProtectedAppShell><MutationFeedback error={params.erreur} /><ContractFormPreview action={createLeaseAction} basePath="/contrats" dashboardHref="/espace" finalizeDocuments={finalizeLeaseDocumentsAction} prepareUploads={prepareLeaseDocumentUploadsAction} rollbackLease={rollbackLeaseAction} tenantOptions={tenantOptions} unitOptions={unitOptions} /></ProtectedAppShell>;
}
